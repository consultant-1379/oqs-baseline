...
# Run Smoke Tests
cd oqs-smoketests
./prepare_db_for_smoke_tests.sh oqstest_default

echo 'rm -rf allure-results folder'
sudo rm -rf allure-results
echo 'rm -rf images folder'
sudo rm -rf images
...
then
    ...
    time docker-compose down --volumes --remove-orphans
    exit 1
fi
time docker-compose down --volumes --remove-orphans
