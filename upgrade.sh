#!/bin/bash
# This script is used to upgrade OQS.
# Note:
# - Has to be run outside the container and from this directory.
# - The arguments to provide when running an upgrade: ./upgrade.sh <baseline_version> <client_version> <server_version> <helpdocs_version> <apidocs_version>
...
time BASELINE_VERSION=$BASELINE_VERSION CLIENT_VERSION=$CLIENT_VERSION SERVER_VERSION=$SERVER_VERSION HELPDOCS_VERSION=$HELPDOCS_VERSION APIDOCS_VERSION=$APIDOCS_VERSION docker-compose -f docker-compose-production.yml pull
if [[ $? -ne 0 ]]
then
    exit 1
fi
